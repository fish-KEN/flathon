...
class FbsMessage(fbsBase):
    """fbsbuf message type class
    ...
    def __initialize(self):
        ...
        for column_index in range(len(self.__sheet[COLUMN_ROW_INDEX])):
            ...
            if type_name == 'enum':
                ...
                enum = fbsEnum(self.__sheet, column_header, name)
                self.__enums.append(enum)

    def __make_elements(self, fbs_elements: List[fbsBase]):
        syntax = ''
        for fbs_element in fbs_elements:
            syntax += fbs_element.make()
        return syntax
